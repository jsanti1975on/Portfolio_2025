                ...
                if file_item.filename:
                    ...
                    self.wfile.write(f"""
                        ...
                        <body style='font-family:sans-serif;text-align:center;margin-top:50px;'>
                            <h2>âœ… Upload successful: {html.escape(filename)}</h2>
                            <button onclick="window.location.href='/'">ðŸ”™ Back to Dashboard</button>
...
def run(server_class=HTTPServer, handler_class=CustomHTTPRequestHandler, port=8080):
    ...
    logger.info(f"ðŸ›¡ï¸ TryHackMe Server Running: http://0.0.0.0:{port}")
    httpd.serve_forever()
